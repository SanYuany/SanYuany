...
async function initialize(){
 ...
 const stage=$('#stage');
 const entry=document.createElement('button');entry.id='enterSpatial';entry.type='button';entry.className='spatial-entry';entry.textContent='360°の家に入る ↗';$('#startExperience').after(entry);
 ...
 if(!reduced&&scrollY<4&&!location.hash){introActive=true;const start=performance.now();function intro(t){if(!introActive)return;app.seek(entryProgress(t-start),true);if(t-start<8000)introRaf=requestAnimationFrame(intro);else stopIntro();}introRaf=requestAnimationFrame(intro);}
}
