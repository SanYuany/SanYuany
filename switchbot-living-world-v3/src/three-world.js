...
export class SwitchBotWorld {
  ...
  createLighting(){
    ...
    this.sun.position.set(-8,14,10); this.sun.castShadow=true;
    this.sun.shadow.mapSize.set(2048,2048); this.sun.shadow.camera.near=0.1; this.sun.shadow.camera.far=50;
    this.sun.shadow.camera.left=-18; this.sun.shadow.camera.right=18; this.sun.shadow.camera.top=18; this.sun.shadow.camera.bottom=-18;
  ...
  box(name,size,pos,mat,group=this.scene){
    const mesh=new THREE.Mesh(new THREE.BoxGeometry(...size),mat); mesh.name=name; mesh.position.set(...pos); mesh.castShadow=true; mesh.receiveShadow=true; group.add(mesh); return mesh;
  }
  cyl(name,r,h,pos,mat,group=this.scene){
    const mesh=new THREE.Mesh(new THREE.CylinderGeometry(r,r,h,28),mat); mesh.name=name; mesh.position.set(...pos); mesh.castShadow=true; mesh.receiveShadow=true; group.add(mesh); return mesh;
  }
  ...
  createHouse(){
    ...
    const ground=this.box('ground',[42,.25,34],[0,-.2,0],this.mat('#7d8d72',1),root); ground.receiveShadow=true;
    this.box('driveway',[13,.12,11],[9,-.05,6],this.mat('#a8aaa5',1),root);
    for(let i=0;i<7;i++){
      const trunk=this.cyl('tree',.16,2.4,[-10+i*3,-.0,-7+(i%2)*1.5],wood,root); trunk.position.y=1.0;
      const crown=new THREE.Mesh(new THREE.IcosahedronGeometry(1.05+(i%3)*.18,2),this.mat(i%2?'#69835e':'#5a7853',1)); crown.position.set(trunk.position.x,2.7,trunk.position.z); crown.castShadow=true; root.add(crown);
    }
    ...
    const doorPivot=new THREE.Group(); doorPivot.position.set(3.55,0,3.88); root.add(doorPivot); this.refs.doorPivot=doorPivot;
    const door=this.box('entrance-door',[1.6,2.65,.18],[.8,1.4,0],wood,doorPivot); door.castShadow=true;
    const lock=this.box('lock-ultra',[.18,.48,.18],[1.36,1.48,-.18],dark,doorPivot); this.refs.lock=lock;
    const lockLed=new THREE.Mesh(new THREE.SphereGeometry(.055,18,12),new THREE.MeshStandardMaterial({color:'#5ad991',emissive:'#5ad991',emissiveIntensity:2.4})); lockLed.position.set(1.37,1.6,-.285); doorPivot.add(lockLed); this.refs.lockLed=lockLed;
    ...
    const hubScreen=this.box('hub-screen',[.76,.47,.02],[3.7,1.27,2.32],blue,root); hubScreen.material.emissive=new THREE.Color('#3b9cc8'); hubScreen.material.emissiveIntensity=.45;

    this.box('sofa-base',[3.3,.55,1.25],[-2.6,.45,-.8],sofa,root);
    this.box('sofa-back',[3.3,1.05,.25],[-2.6,1.05,-1.35],sofa,root);
    this.box('coffee-table',[2.0,.12,1.05],[-2.6,.72,1.0],wood,root); this.box('coffee-leg1',[.14,.62,.14],[-3.35,.38,.7],dark,root); this.box('coffee-leg2',[.14,.62,.14],[-1.85,.38,1.3],dark,root);
    this.box('tv-console',[3.3,.38,.48],[-3.0,.38,-3.55],dark,root);
    const tv=this.box('tv',[2.7,1.5,.08],[-3.0,1.4,-3.7],dark,root); tv.material.emissive=new THREE.Color('#233842'); tv.material.emissiveIntensity=.25;
    ...
    this.box('pillow1',[1.1,.18,.55],[2.1,4.16,-.55],fabric,root); this.box('pillow2',[1.1,.18,.55],[3.5,4.16,-.55],fabric,root);
    ...
    this.refs.curtainLeft=cl; this.refs.curtainRight=cr;
    this.cyl('curtain3-motor',.11,.28,[.9,5.63,-3.7],dark,root).rotation.z=Math.PI/2;

    const fixture=(name,pos)=>{ const m=new THREE.Mesh(new THREE.SphereGeometry(.16,20,12),glowMat.clone()); m.position.set(...pos); root.add(m); const p=new THREE.PointLight(0xffd18a,0,8,2); p.position.set(...pos); p.castShadow=true; root.add(p); this.refs[name]={mesh:m,light:p}; };
    fixture('bedLight',[2.75,5.75,-.1]); fixture('entryLight',[3.8,2.65,2.15]); fixture('livingLight',[-2.2,2.6,.0]);

    this.box('step',[2.8,.22,1.5],[4.1,.08,4.7],this.mat('#9a9c99',1),root);
    for(let i=0;i<4;i++){ const shrub=new THREE.Mesh(new THREE.IcosahedronGeometry(.55+.1*i,2),this.mat('#607c58',1)); shrub.position.set(-5+i*1.35,.45,4.75); shrub.castShadow=true; root.add(shrub); }
  }
  ...
  keyframe(progress){
    ...
    const pos=new THREE.Vector3(); pos.lerpVectors(pa,pb,t);
    const target=new THREE.Vector3(); target.lerpVectors(ta,tb,t);
    return {pos,target};
  }
