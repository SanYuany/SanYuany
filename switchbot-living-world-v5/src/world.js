...
import {getCamera,getState,roomViews,clamp,getResidentPose} from './timeline.js';
...
export class LivingWorld {
 constructor(canvas,{reducedMotion=false,onFrame=()=>{}}={}){
  ...
  this.scene.fog=new T.Fog('#eee9df',42,94);this.preview={};this.projectionShift=NaN;
  ...
  this.camera=new T.PerspectiveCamera(43,1,.065,150);this.look=new T.Vector3();
  const envScene=new RoomEnvironment();const pmrem=new T.PMREMGenerator(this.renderer);this.envTarget=pmrem.fromScene(envScene,.035);this.scene.environment=this.envTarget.texture;this.scene.environmentIntensity=.46;envScene.dispose();pmrem.dispose();
 ...
 resize(){
  ...
  this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,this.mobile?1.25:1.5));this.camera.fov=this.mobile?46:43;this.renderer.setSize(w,h,false);
  this.camera.aspect=w/h;this.width=w;this.height=h;this.projectionShift=NaN;this.camera.clearViewOffset();this.camera.updateProjectionMatrix();this.dirty=true;
 }
 ...
 setCamera(frame){
  this.camera.position.set(...frame.position);this.look.set(...frame.target);this.camera.lookAt(this.look);
  // The illustration reserves headline space only in the establishing shot. Interior cameras are centered.
  const ratio=this.mobile?0:(-.10*(1-Math.min(1,this.progress/.13))),y=this.mobile?.055:0;
  if(this.width && (ratio!==this.projectionShift||this.projectionMode!==this.mode)){this.camera.setViewOffset(this.width,this.height,this.width*ratio,this.height*y,this.width,this.height);this.projectionShift=ratio;this.projectionMode=this.mode;}
 }
 setPreview(key,value){if(!['curtain','living','entry'].includes(key))return;this.preview[key]=value?1:0;this.stateSignature='';this.dirty=true;}
 resetPreview(){this.preview={};this.stateSignature='';this.dirty=true;}

 setMode(mode){this.mode=mode;this.preview={};this.stateSignature='';this.controls.enabled=mode==='explore';this.canvas.style.touchAction=mode==='explore'?'none':'pan-y';this.dirty=true;if(mode==='explore'){this.camera.clearViewOffset();this.camera.updateProjectionMatrix();this.controls.target.copy(this.look);this.setRoom('all');}else{this.roomTransition=null;this.setFloor('all');}}
 ...
 applyState(s){
 if(this.mode==='explore')s={...s,...this.preview};
 const key=this.mode+'|'+s.p+'|'+JSON.stringify(this.preview);if(key===this.stateSignature)return;this.stateSignature=key;
  for(const c of this.house.curtains){const scale=1-.81*s.curtain;c.group.scale.x=scale;c.motor.scale.x=1/scale;}
  ...
  const day=new T.Color('#ede9df'),evening=new T.Color('#d7ccbf'),night=new T.Color('#27343d');const morningSky=Math.sin(Math.PI*clamp((s.p-.10)/.175))**2;const bg=day.clone().lerp(new T.Color('#bed7df'),morningSky).lerp(evening,clamp(s.returning)).lerp(night,s.night);
  ...
  this.sun.intensity=3.3*(1-s.night)+.20;this.sun.color.set(s.returning>.1?'#ffd6a7':'#fff3de');this.hemi.intensity=.58*(1-s.night)+.26;this.fill.intensity=.30*(1-s.night)+.18;
  this.scene.environmentIntensity=.38*(1-s.night)+.16;
  this.house.sunPatch.material.opacity=.65*s.curtain*(1-s.night);
  this.house.sunPatch.scale.x=.16+.84*s.curtain;
  this.updateResident(s);
  this.state=s;const shadowKey=[s.door.toFixed(2),s.curtain.toFixed(2),roof.toFixed(2),s.night.toFixed(1),this.floor].join('|');if(shadowKey!==this.shadowSignature){this.renderer.shadowMap.needsUpdate=true;this.shadowSignature=shadowKey;}
 }
 updateResident(s){
  const person=this.house.resident;if(!person)return;
  const pose=getResidentPose(s.p);person.visible=this.mode==='story'&&pose.visible;if(!person.visible)return;
  person.position.set(...pose.position);person.rotation.y=pose.angle;
  const walk=Math.sin(pose.phase*Math.PI*7)*.30;person.userData.limbs.legL.rotation.x=walk;person.userData.limbs.legR.rotation.x=-walk;
  person.userData.limbs.arm.rotation.x=pose.reaching?-1.0:walk*.5;
 }
 animate(time){
