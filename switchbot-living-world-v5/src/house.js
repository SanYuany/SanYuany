...
import {addLivingDetails} from './living-details.js';
import {compactHouse} from './optimize.js';
...
export function createHouse(){
 ...
 box('2F oak floor',8.7,.19,7,.45,3.22,0,oak,f2);
 box('stair landing north',.9,.19,.88,-4.35,3.22,-3.06,oak,f2);
 box('stair landing south',.9,.19,2.1,-4.35,3.22,2.45,oak,f2);
 ...
 const sofa=new T.Group();sofa.position.set(-3.23,.26,1.57);sofa.rotation.y=Math.PI;f1.add(sofa);
 ...
 const table=cyl('round coffee table',.67,.09,-2.98,.71,-.02,walnut,f1);table.scale.x=1.28;
 for(const x of [-3.23,-2.28])rod([x,.3,-.22],[x,.68,-.02],.032,ink,f1);
 cyl('ceramic vase',.095,.23,-3.0,.87,-.06,mat('vase','#dfd4bc'),f1,.06);rod([-3.0,.99,-.06],[-2.92,1.32,-.05],.006,walnut,f1);
 box('book',.32,.035,.22,-3.32,.78,.11,mat('book','#d6cbb6'),f1,.007);
 ...
 for(const [x,y,z,w,d,parent] of [[-3.23,.304,1.57,3.45,1.8,f1],[-2.98,.31,-.02,2.0,1.4,f1],[-.32,.263,.32,2.4,2.8,f1],[4.46,.259,1.7,1.0,1.9,f1],[2.02,3.349,-.68,2.75,2.8,f2],[.48,3.353,-1.3,.8,.8,f2],[3.56,3.353,-1.3,.8,.8,f2]]){const m=new T.Mesh(new T.PlaneGeometry(w,d),aoMat);m.name='furniture contact shadow';m.rotation.x=-Math.PI/2;m.position.set(x,y,z);parent.add(m);}
 refs.materials=materials;refs.box=box;addLivingDetails(refs,{box,cyl,rod,ball,mat,oak,walnut,plaster,ink,cream,sage});compactHouse(refs);return refs;
}
