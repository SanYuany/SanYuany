...
test('clean checkout builds without an untracked empty assets folder',()=>{
 ...
 const receipt=JSON.parse(readFileSync(path.join(dir,'dist/release.json'),'utf8'));assert.equal(receipt.version,JSON.parse(readFileSync(path.join(dir,'package.json'),'utf8')).version,'release identity must match package version');assert.match(receipt.files['src/world.js'],/^[a-f0-9]{64}$/);assert.ok(existsSync(path.join(dir,'dist/src/export.js')));
