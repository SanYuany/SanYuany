...
try{await page.goto('http://127.0.0.1:4198/',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>window.__spatial?.ready&&window.__cinematic?.ready&&window.__livingWorld.frames>0,null,{timeout:60000});await page.evaluate(()=>{window.__spatial.stopIntro();window.__spatial.stopOrbit();window.__app.stopPlay();window.__livingWorld.active=false;});
 ...
 const output=path.join(out,'SwitchBot-Living-World-V5.2.1-'+label+'.mp4');const r=spawnSync('ffmpeg',['-y','-framerate','16','-i',path.join(frames,'%04d.jpg'),'-c:v','libx264','-crf','19','-pix_fmt','yuv420p','-movflags','+faststart',output],{encoding:'utf8'});if(r.status!==0)throw Error(r.stderr);
 ...
 fs.writeFileSync(path.join(out,label+'-manifest.json'),JSON.stringify({version:'5.2.1',sourceSha:process.env.GITHUB_SHA,renderer:'Actual Three.js canvas rendered at sequential deterministic scroll positions',independentPortraitCamera:mobile,width,height,frames:count,durationSeconds:16,playbackFPS:16,realTimePerformanceBenchmark:false,decode:'PASS'},null,2));fs.rmSync(frames,{recursive:true,force:true});console.log('FILM_COMPLETE',output);
